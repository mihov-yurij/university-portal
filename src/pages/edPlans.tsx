...
const ProgramSection = ({ data, activeDoc, onLoadPdf }: { data: ProgramData, activeDoc: string | null, onLoadPdf: (f: string) => void }) => (
  <section className="mb-14">
    ...
    <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm mb-8">
      ...
      <p className="text-slate-500 text-[15px] leading-relaxed text-justify whitespace-pre-line font-medium">{data.guarantor.bio}</p>
    </div>
...
export default function EdPlans() {
  ...
  async function loadPdf(fileName: string) {
    ...
    try {
      const loadingTask = pdfjsLib.getDocument(`/docs/${fileName}`);
...
//         bio:` "Гарант програми «Бізнес на морському транспорті» (Морський бізнес)
// Наврозова Юлія Олександрівна
// Кандидат економічних наук, доцент, завідувачка кафедри морського бізнесу та
// маркетингу
// Народилася в місті-герої Одеса в родині співробітників Чорноморського морського
// пароплавства. У 2001 р. закінчила факультет економіки та управління Одеського
// національного морського університету за спеціальністю «Менеджмент організацій»,
// отримала диплом з відзнакою. У 2005 році захистила кандидатську дисертацію за
// спеціальністю «Економіка транспорту та зв&#39;язку» на тему «Економічні основи управління
// якістю продукції морських торговельних портів У країни». З 2004 року почала свою
...
// «Укрпорт».`
      ...
      studyPlans: [{ year: "2024", fullTime: "plan_mng_24_d.pdf", partTime: "plan_mng_24_z.pdf" },
        { year: "2024", fullTime: "plan_mng_24_d.pdf", partTime: "plan_mng_24_z.pdf" }
      ]
    ...
    },
    {
      ...
      items: [
        ...
        { header: "РЕЦЕНЗІЇ" },
        { name: '3. Рецензія-відгук на ОП "Маркетинг"', file: "rev1_mkt_24.pdf" },
        { name: '4. Рецензія-відгук на ОП "Маркетинг"', file: "rev2_mkt_24.pdf" },
      ],
      guarantor: {
